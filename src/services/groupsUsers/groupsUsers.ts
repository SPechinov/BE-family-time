...
export class GroupsUsersService implements IGroupsUsersService {
  ...
  async findUserGroups(userId: UUID, options?: { client?: PoolClient }): Promise<GroupsUsersEntity[]> {
    return this.findMany(new GroupsUsersFindManyEntity({ userId }), options);
  }

  async findGroupUsers(groupId: UUID, options?: { client?: PoolClient }): Promise<GroupsUsersEntity[]> {
    return this.findMany(new GroupsUsersFindManyEntity({ groupId }), options);
  }

  async findGroupOwners(groupId: UUID, options?: { client?: PoolClient }): Promise<GroupsUsersEntity[]> {
