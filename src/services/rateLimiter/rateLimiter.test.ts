...
describe('RateLimiterService', () => {
  ...
  describe('checkLimitOrThrow', () => {
    ...
    it('should allow request when no previous attempts', async () => {
      ...
      expect(mockRedis.hSet).toHaveBeenCalledWith(`rate-limit:${mockConfig.prefix}:${validKey}`, 'lastTime', expect.any(Number));
    ...
    it('should throw ErrorTooManyRequests when attempts exceed maxAttempts', async () => {
      ...
      expect(mockRedis.hmGet).toHaveBeenCalledWith(`rate-limit:${mockConfig.prefix}:${validKey}`, ['attempts', 'lastTime']);
    ...
    it('should normalize time correctly', async () => {
      ...
      expect(mockRedis.hSet).toHaveBeenCalledWith(`rate-limit:${mockConfig.prefix}:${validKey}`, 'lastTime', expectedTime);
      expect(mockRedis.expire).toHaveBeenCalledWith(`rate-limit:${mockConfig.prefix}:${validKey}`, 60);
    });
  });
});