...
const startHashPasswordServiceEncrypt = () => {
  hashPasswordService.hashPassword('test-password');
};

export const benchmarkHashPasswordServiceEncrypt = () => {
  const { avg, median } = benchmark(startHashPasswordServiceEncrypt, 100);
  console.log(`\nAverage: ${avg}, Median: ${median}`);
};

const startHashPasswordServiceDescrypt = () => {
...
export const benchmarkHashPasswordServiceDecrypt = () => {
  const { avg, median } = benchmark(startHashPasswordServiceDescrypt, 100);
  console.log(`\nAverage: ${avg}, Median: ${median}`);
};
